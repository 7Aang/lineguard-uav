...
async def _execute_mavsdk_vehicle(
    ...
    try:
        ...
        from mavsdk.offboard import (  # type: ignore[import-not-found]
            OffboardError,
            PositionNedYaw,
        )
    except ImportError:
...
async def _execute_uav_mission_once(
    task_id: str,
    plan: MissionPlan,
...
def _execution_contract(plan, backend):
    from lineguard.reliability import contract_hash

    settings = get_settings()
    limits = {
        "altitude": settings.maximum_altitude_m,
        "radius": settings.maximum_radius_m,
        "separation": settings.minimum_uav_separation_m,
        "tolerance": settings.waypoint_tolerance_m,
        "timeout": settings.waypoint_timeout_s,
        "connections": settings.px4_connections,
    }
    return contract_hash(plan.model_dump(mode="json"), backend.value, limits)


def approve_mission(task_id: str, plan: MissionPlan, reviewer: str, backend=None):
    from lineguard.reliability import ExecutionLedger

    settings = get_settings()
    selected = backend or settings.uav_backend
    digest = _execution_contract(plan, selected)
    ExecutionLedger(settings.data_dir / "execution_ledger.sqlite3").approve(
        task_id, digest, reviewer
    )
    return digest


@audited_tool("execute_uav_mission")
async def execute_uav_mission(task_id: str, plan: MissionPlan, backend: UAVBackend | None = None):
    from lineguard.reliability import DispatchDenied, ExecutionLedger

    settings = get_settings()
    selected = backend or settings.uav_backend
    check = validate_uav_mission(task_id, plan)
    if not check.approved:
        return _failed_execution(selected, check, "MISSION_SAFETY_REJECTED", 0)
    ledger = ExecutionLedger(settings.data_dir / "execution_ledger.sqlite3")
    digest = _execution_contract(plan, selected)
    try:
    ...
    if cached:
        ...
        return result
    try:
        result = await _execute_uav_mission_once(task_id, plan, selected)
        result.execution_contract = digest
        ledger.finish(task_id, result.model_dump(mode="json"))
        return result
    except BaseException as exc:
        ledger.uncertain(task_id, type(exc).__name__)
        raise
